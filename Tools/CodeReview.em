...
macro Review_Add_Comment()
{
    ...
    promote = "类型 : d(Defect缺陷); q(Query疑问)"
    sTemp = ask(promote);
    ...
    if(sTemp == "Defect缺陷")
    {
        ...
        /* get Categories */
        promote = "类别 : s(需求); h(概要设计); l(详细设计); t(测试计划); c(代码); u(用户手册)"
        sTemp = ask(promote);
        sTemp = toupper(sTemp[0]);
        ...
        /* get defect type */
        promote = "缺陷类型 : i(接口); f(功能); b(构建/打包); a(赋值); d(文档); c(校验); l(算法); t(时序/顺序缺陷); O,o(其它)"
        sTemp = ask(promote);
        sTemp = toupper(sTemp[0]);
